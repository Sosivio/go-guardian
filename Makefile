...
lint-fix: 
	@FILES="$(shell find . -type f -name '*.go' -not -path "./vendor/*")"; goimports -local "github.com/Sosivio/go-guardian/v2" -w $$FILES
...
release: 
	git clean -df 
	git checkout -- .
	$(shell ./bin/semantic-release --slug Sosivio/go-guardian) 